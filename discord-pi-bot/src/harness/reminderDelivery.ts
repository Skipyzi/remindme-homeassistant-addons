...
export function deliveryTargets(reminder: Reminder, ownerId: string): ReminderDeliveryStatus {
	const isOwner = Boolean(ownerId) && reminder.userId === ownerId;
	return { homeAssistant: isOwner ? "pending" : "skipped", mobile: isOwner ? "pending" : "skipped", discord: "pending" };
}
...
export async function deliverReminder(
	...
	const next = { ...status };
	async function attempt(key: keyof ReminderDeliveryStatus, operation?: () => Promise<void>) {
		if (next[key] === "delivered" || next[key] === "skipped") return;
		if (!operation) { next[key] = "skipped"; return; }
		try { await operation(); next[key] = "delivered"; } catch { next[key] = "failed"; }
	}
	await attempt("homeAssistant", () => adapters.homeAssistant(reminder));
	await attempt("mobile", adapters.mobile ? () => adapters.mobile?.(reminder) as Promise<void> : undefined);
