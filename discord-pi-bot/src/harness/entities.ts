...
export function normalizeEntity(entity: HassEntity): EntityCard {
	...
	return {
		...
		area: typeof entity.attributes.area_name === "string" ? entity.attributes.area_name : undefined,
		capabilities: {
			toggle: ["light", "switch", "fan", "media_player"].includes(domain),
			brightness: domain === "light" && modes.some((mode) => mode !== "onoff"),
			colorTemperature: domain === "light" && modes.some((mode) => ["color_temp", "rgbww", "rgbw"].includes(mode)),
			color: domain === "light" && modes.some((mode) => ["rgb", "rgbw", "rgbww", "hs", "xy"].includes(mode)),
		},
		brightness: typeof entity.attributes.brightness === "number" ? entity.attributes.brightness : undefined,
		colorTemperature: typeof entity.attributes.color_temp_kelvin === "number" ? entity.attributes.color_temp_kelvin : undefined,
		rgbColor: Array.isArray(entity.attributes.rgb_color) ? entity.attributes.rgb_color.map(Number) : undefined,
