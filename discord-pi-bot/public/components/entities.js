(function exposeEntities(globalScope) {
  async function performEntityAction(entity, action, value) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || "Home Assistant rejected the action");
      if (result.confirmation_required) return { confirmation: result, previous };
      Object.assign(entity, result, { pending: false, applied: true });
      setTimeout(() => { entity.applied = false; }, 900);
      return { entity };
    } catch (error) {
      Object.assign(entity, previous, { pending: false, error: error.message || String(error) });
