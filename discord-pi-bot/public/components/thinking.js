(function exposeThinking(globalScope) {
  function formatThought(entry) {
    const durationMs = Math.max(0, (entry.completedAt || Date.now()) - (entry.startedAt || Date.now()));
    const duration = durationMs >= 60_000 ? `${(durationMs / 60_000).toFixed(1)} minutes` : `${(durationMs / 1_000).toFixed(1)} seconds`;
