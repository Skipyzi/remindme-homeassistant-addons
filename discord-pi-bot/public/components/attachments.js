	...
	async function addFiles(app, files) {
		for (const file of [...files].slice(0, 4 - app.attachments.length)) {
			if (!["image/jpeg", "image/png", "image/webp"].includes(file.type)) {
				app.attachmentError = "Use JPEG, PNG, or WebP images.";
				continue;
			}
			if (file.size > 6_000_000) {
				app.attachmentError = "Each image must be smaller than 6 MB.";
				continue;
			}
		...
		if (app.attachments.length && !app.visionEnabled)
			app.attachmentError =
				"The active model cannot process images. Change models or remove the attachment.";
	}
